...
def _default_strategy(model, bayesian_layer, **kwargs):
  ...
  conv_list = kwargs['conv_list'] if 'conv_list' in kwargs else {tf.keras.layers.Conv2D}
  supported_layers = {}
  for i, layer in enumerate(model.layers):
    if type(layer) in conv_list or isinstance(layer, tf.keras.layers.Dense):
...
def _last_strategy(model, bayesian_layer, **kwargs):
  ...
  for i, layer in enumerate(model.layers):
    if type(layer) in conv_list:
      last_conv = i
      after_conv = True
    if (isinstance(layer, tf.keras.layers.Dense) and after_conv):
      count = i - 1
      after_conv = False
  if (last_conv < 0):
    return supported_layers
...
def _full_strategy(model, bayesian_layer, **kwargs):
  ...
  supported_layers = {}
  for i, layer in enumerate(model.layers):
    if isinstance(layer, tf.keras.engine.input_layer.InputLayer):
      continue
    if type(layer) in conv_list or isinstance(layer, tf.keras.layers.Dense):
      supported_layers[i-1] = bayesian_layer
  return supported_layers
