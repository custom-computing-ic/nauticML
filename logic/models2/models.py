...
from keras.layers import Input, Dense, Activation, \
                         ...
                         Conv2D, AveragePooling2D,  MaxPool2D, GlobalAveragePooling2D
...
def lenet(args, num_layers):
    ...
    # Lenet
    # Convolutional layer
    model.add(Conv2D(filters=int(20*args.scale_factor), kernel_size=(5,5), \
    ...
    model.compile(optimizer=SGD(lr = args.lr), loss=['categorical_crossentropy'], \
                  metrics=['accuracy'])
    return model
...
def ResNet18(classes, input_shape, args, num_bayes_loc=8, weight_decay=1e-4, base_filters=64):
    ...
    x = conv2d_bn_relu(x, filters=new_base_filters, kernel_size=(3, 3), \
                       weight_decay=weight_decay, strides=(1, 1))

    # # conv 2
    x = ResidualBlock(x, filters=new_base_filters, kernel_size=(3, 3), \
                      weight_decay=weight_decay, downsample=False)
    ...
    # # conv 3
    x = ResidualBlock(x, filters=2*new_base_filters, kernel_size=(3, 3), \
                      weight_decay=weight_decay, downsample=True)
    # Insert Bayeian Layer, can be mc dropout or mask ensumble layer
    if (num_nonbayes_layer < 0): x = Insert_Bayesian_Layer(args, x)
    num_nonbayes_layer -= 1
    ...
    # # conv 4
    x = ResidualBlock(x, filters=4*new_base_filters, kernel_size=(3, 3), \
                      weight_decay=weight_decay, downsample=True)
    # Insert Bayeian Layer, can be mc dropout or mask ensumble layer
    if (num_nonbayes_layer < 0): x = Insert_Bayesian_Layer(args, x)
    num_nonbayes_layer -= 1
    ...
    # # conv 5
    x = ResidualBlock(x, filters=8*new_base_filters, kernel_size=(3, 3), \
                      weight_decay=weight_decay, downsample=True)
    # Insert Bayeian Layer, can be mc dropout or mask ensumble layer
    if (num_nonbayes_layer < 0): x = Insert_Bayesian_Layer(args, x)
    num_nonbayes_layer -= 1
    ...
    model.compile(optimizer=SGD(lr=args.lr, momentum=0.9, nesterov=False), \
                  loss=['categorical_crossentropy'], metrics=['accuracy'])
    # model.compile(optimizer=Adam(lr=args.lr, amsgrad=True), \
    #               loss=['categorical_crossentropy'], metrics=['accuracy'])
    return model
...
def ResNetForCIFAR10(classes, name, input_shape, block_layers_num, weight_decay, lr):
    ...
    base_filters = 16
    x = input
    x = conv2d_bn_relu(x, filters=base_filters, kernel_size=(3, 3), \
                       weight_decay=weight_decay, strides=(1, 1))

    # # conv 2
    for i in range(block_layers_num):
        x = ResidualBlock(x, filters=base_filters, kernel_size=(3, 3), \
                          weight_decay=weight_decay, downsample=False)
    # # conv 3
    x = ResidualBlock(x, filters=base_filters*2, kernel_size=(3, 3), \
                      weight_decay=weight_decay, downsample=True)
    for i in range(block_layers_num - 1):
        x = ResidualBlock(x, filters=base_filters*2, kernel_size=(3, 3), \
                          weight_decay=weight_decay, downsample=False)
    # # conv 4
    x = ResidualBlock(x, filters=base_filters*4, kernel_size=(3, 3), \
                      weight_decay=weight_decay, downsample=True)
    for i in range(block_layers_num - 1):
        x = ResidualBlock(x, filters=base_filters*4, kernel_size=(3, 3), \
                          weight_decay=weight_decay, downsample=False)
    ...
    model.compile(optimizer=SGD(lr=lr, momentum=0.9, nesterov=False), \
                  loss=['categorical_crossentropy'], metrics=['accuracy'])
    return model
